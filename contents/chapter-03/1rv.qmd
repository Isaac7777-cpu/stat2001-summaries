...
Suppose $Y \sim \text{NegBin}(r, p)$, then,

\begin{align*}
E(e^{Yt}) & = \sum_{y = r}^\infty e^{yt} \binom{y-1}{r-1} p^r q^{y-r} \\
& = \sum_{y = r}^\infty e^{yt} \frac{(y - 1)!}{(r - 1)! (y - r)!} p^r q^{y-r} \\
& = p^r \sum_{y = r}^\infty 
\end{align*}
...
The direct proof is relatively hard to see. However, using @thm-geometric-mgf with @thm-mgf-of-list-of-independent-vars.
